...
import {
  ...
  DirectorControl,
  ManagerControl,
} from '../../components/GeneralComponents/AdminRoute';
...
function AssignTripForm() {
  ...
  const [selectedMintika, setSelectedMintika] = useState(
    user.role === 'director' || user.role === 'manager' ? user.mintika_id : ''
  );
  ...
  const filteredKurumlar = selectedMintika
    ...
    : isDirector
      ? kurumList.filter((k) => k.mintika_id == user.mintika_id)
      : kurumList;

  const filteredUsers = selectedKurum ? userList.filter((u) => u.kurum_id == selectedKurum) : []; // Kurum seçilmeden kullanıcı gösterme
        ...
        <form className="row g-3" onSubmit={handleSubmit}>
          ...
          {isAdmin && (
            <div className="col-md-6">
              ...
              <select
                ...
                value={selectedMintika}
                onChange={handleMintikaChange}
              >
          ...
          {(isAdmin || isDirector) && (
            <div className="col-md-6">
              ...
              <select
                ...
                disabled={isAdmin && !selectedMintika}
              >
          ...
          <div className="col-md-6">
            <label htmlFor="userId" className="form-label">
              Kullanıcı <span className="text-danger">*</span>
            </label>
            <select
              id="userId"
              name="userId"
          ...
          <div className="col-md-6">
            <label htmlFor="vehicleId" className="form-label">
              Araç <span className="text-danger">*</span>
            </label>
            <select
              id="vehicleId"
              name="vehicleId"
          ...
          <div className="col-12 text-center">
            <button type="submit" className="btn btn-primary">
              Atamayı Tamamla ve Seyehati Başlat
            </button>
          </div>
        </form>
      </div>
